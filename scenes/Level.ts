...
class Level extends Phaser.Scene {
	...
	editorCreate() {
		...
		// mirror_bg
		const mirror_bg = this.add.rectangle(273, 251, 128, 128);
		mirror_bg.scaleX = 3.4764732519125743;
		mirror_bg.scaleY = 3.2089789491501506;
		mirror_bg.isFilled = true;
		mirror_bg.fillColor = 7627610;
		...
		// mirror_scene_frame
		const mirror_scene_frame = this.add.image(0, 0, "mirror_scene_frame");
		mirror_scene_frame.setOrigin(0, 0);
		...
		// request_container
		const request_container = this.add.container(600, 200);
		request_container.scaleX = 0.5;
		request_container.scaleY = 0.5;
		
		// sprite
		const sprite = this.add.sprite(0, 0, "thought_bubble_1");
		sprite.scaleX = 1.8;
		sprite.scaleY = 1.8;
		...
		// customer_foreground
		const customer_foreground = this.add.image(293, 274, "customer_steve_head_back");
		...
		timerContainerTimer.timer_length = 20;
	...
	private mirror_bg: Phaser.GameObjects.Rectangle|undefined;
	...
	private current_timeline: Phaser.Tweens.Timeline;
	...
	introCustomer() {
		...
		timeline.add({
			...
			onStart: (tween, target) => {
				this.request_container.x = 600;
				this.request_container.y = 200;
	...
	onGameTimerComplete() {
		...
		if (this.current_timeline) {
			this.current_timeline.stop();
			this.request_container.visible = false;
		}
		// Get the score and use it later gator
		...
		timeline.add({
			...
			onStart: () => {
				...
				this.customer_container.x = this.customer_foreground.x;
				this.customer_container.y = this.customer_foreground.y;
				this.customer_container.setDepth(1);
	...
	createNewCustomer() {
		...
		this.customer_container.setPosition(68, 33);
	}
